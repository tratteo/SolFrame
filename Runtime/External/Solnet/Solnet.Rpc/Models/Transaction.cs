...
namespace Solnet.Rpc.Models
{
    ...
    public class Transaction
    {
        ...
        /// <summary>
        ///   The nonce information of the transaction. <remarks> When this is set, the <see cref="NonceInformation"/>'s Nonce is used as
        ///   the <c> RecentBlockhash </c>. </remarks>
        /// </summary>
        public NonceInformation NonceInformation { get; set; }

        /// <summary>
        ///   The signatures for the transaction. <remarks> These are typically created by invoking the <c> Build(IList{Account} signers)
        ///   </c> method of the <see cref="TransactionBuilder"/>, but can be created by deserializing a Transaction and adding signatures
        ///   manually. </remarks>
        /// </summary>
        ...
        public static Transaction Populate(Message message, IList<byte[]> signatures = null)
        {
            ...
            for (var i = 0; i < message.Instructions.Count; i++)
            {
                var compiledInstruction = message.Instructions[i];
                (var accountLength, _) = ShortVectorEncoding.DecodeLength(compiledInstruction.KeyIndicesCount);
        ...
        public static Transaction Deserialize(ReadOnlySpan<byte> data)
        {
            ...
            (var signaturesLength, var encodedLength) =
        ...
        /// <summary>
        ///   Sign the transaction with the specified signers. Multiple signatures may be applied to a transaction. The first signature is
        ///   considered primary and is used to identify and confirm transaction. <remarks>
        ///   <para> If the transaction <c> FeePayer </c> is not set, the first signer will be used as the transaction fee payer account. </para>
        ///   <para>
        ///     Transaction fields SHOULD NOT be modified after the first call to <c> Sign </c> or an externally created signature has been
        ///     added to the transaction object, doing so will invalidate the signature and cause the transaction to be rejected by the cluster.
        ///   </para>
        ///   <para>
        ...
        {
            Signatures ??= new List<SignaturePubKeyPair>();
            var uniqueSigners = DeduplicateSigners(signers);
            var serializedMessage = CompileMessage();

            foreach (var account in uniqueSigners)
            {
                var signatureBytes = account.Sign(serializedMessage);
        ...
        /// <summary>
        ///   Sign the transaction with the specified signer. Multiple signatures may be applied to a transaction. The first signature is
        ///   considered primary and is used to identify and confirm transaction. <remarks>
        ///   <para> If the transaction <c> FeePayer </c> is not set, the first signer will be used as the transaction fee payer account. </para>
        ///   <para>
        ///     Transaction fields SHOULD NOT be modified after the first call to <c> Sign </c> or an externally created signature has been
        ...
        /// <summary>
        ///   Partially sign a transaction with the specified accounts. All accounts must correspond to either the fee payer or a signer
        ///   account in the transaction instructions.
        /// </summary>
        /// <param name="signers"> The signer accounts. </param>
        ...
        /// <summary>
        ///   Partially sign a transaction with the specified account. The account must correspond to either the fee payer or a signer
        ///   account in the transaction instructions.
        /// </summary>
        /// <param name="signer"> The signer account. </param>
        ...
        /// <summary>
        ///   Adds an externally created signature to the transaction. The public key must correspond to either the fee payer or a signer
        ///   account in the transaction instructions.
        /// </summary>
        ...
        {
            var signaturesLength = ShortVectorEncoding.EncodeLength(Signatures.Count);
            var serializedMessage = CompileMessage();
        ...
        private static IEnumerable<Account> DeduplicateSigners(IEnumerable<Account> signers)
        {
            ...
            foreach (var account in signers)
            {
