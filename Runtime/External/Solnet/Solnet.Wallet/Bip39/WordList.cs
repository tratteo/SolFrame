...
namespace Solnet.Wallet.Bip39
{
    ...
    public class WordList
    {
        ...
        public static Language AutoDetectLanguage(IEnumerable<string> words)
        {
            ...
            {
                if (English.WordExists(s, out var _))
                {
                    //english is at 0
                    languageCount[0]++;
                }

                if (Japanese.WordExists(s, out var _))
                {
                    //japanese is at 1
                    languageCount[1]++;
                }

                if (Spanish.WordExists(s, out var _))
                {
                    //spanish is at 2
                    languageCount[2]++;
                }

                if (ChineseSimplified.WordExists(s, out var _))
                {
                    //chinese simplified is at 3
                    languageCount[3]++;
                }

                if (ChineseTraditional.WordExists(s, out var _) && !ChineseSimplified.WordExists(s, out var _))
                {
                    //chinese traditional is at 4
                    languageCount[4]++;
                }
                if (French.WordExists(s, out var _))
                {
                    languageCount[5]++;
                }

                if (PortugueseBrazil.WordExists(s, out var _))
                {
                    //portuguese_brazil is at 6
                    languageCount[6]++;
                }

                if (Czech.WordExists(s, out var _))
                {
                    //czech is at 7
                    languageCount[7]++;
                }
        ...
        public int[] ToIndices(string[] words)
        {
            ...
            {
                if (!WordExists(words[i], out var idx))
                {
