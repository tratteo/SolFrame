...
namespace Solnet.KeyStore
{
    public static class Utils
    {
        ...
        public static byte[] FromStringByteArray(this string data)
        {
            ...
            var index = 0;
            var i = 0;
            var newS = data.ToArray();// .AsSpan(1, data.Length - 1);

            while ((i = newS.ToList().IndexOf(',')) != -1)
            {
                UnityEngine.Debug.Log(newS.Slice(0, i).ToString());
                bytes[index++] = byte.Parse(newS.Slice(0, i).ToString());//[..i]);
                newS = newS.Slice(i + 1);//[(i + 1)..];
            }

            bytes[index] = byte.Parse(newS.Slice(0, newS.Length - 1).ToString());//[..^1]);//newS.Length - 1
            if (index != 63)
