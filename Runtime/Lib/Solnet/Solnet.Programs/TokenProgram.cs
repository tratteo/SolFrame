...
namespace Solnet.Programs
{
    public static class TokenProgram
    {
        ...
        /// <summary>
        ///   Transfers tokens from one account to another either directly or via a delegate. If this account is associated with the native
        ///   mint then equal amounts of SOL and Tokens will be transferred to the destination account.
        /// </summary>
        ...
        {
            return Transfer(Encoders.Base58.DecodeData(source), Encoders.Base58.DecodeData(destination), amount, Encoders.Base58.DecodeData(owner));
        }
            ...
            byte[] source, byte[] destination, long amount, byte[] owner)
        {
            ...
            {
                ProgramId = Encoders.Base58.DecodeData(ProgramId),
                Keys = keys,
        ...
        /// <summary>
        ///   <para>
        ///     Transfers tokens from one account to another either directly or via a delegate. If this account is associated with the
        ///     native mint then equal amounts of SOL and Tokens will be transferred to the destination account.
        ///   </para>
        ///   <para>
        ///     This instruction differs from Transfer in that the token mint and decimals value is checked by the caller. This may be
        ///     useful when creating transactions offline or within a hardware wallet.
        ///   </para>
        /// </summary>
        /// <param name="source"> The account to transfer tokens from. </param>
        /// <param name="destination"> The account to transfer tokens to. </param>
            ...
            string source, string destination, long amount, byte decimals, string owner, string tokenMint)
        {
            return TransferChecked(
                Encoders.Base58.DecodeData(source),
                Encoders.Base58.DecodeData(destination),
                amount,
                decimals,
                Encoders.Base58.DecodeData(owner),
                Encoders.Base58.DecodeData(tokenMint));
        }
        ...
        /// <summary>
        ///   <para>
        ///     Initializes a new account to hold tokens. If this account is associated with the native mint then the token balance of the
        ///     initialized account will be equal to the amount of SOL in the account. If this account is associated with another mint, that
        ///     mint must be initialized before this command can succeed.
        ///   </para>
        ///   <para>
        ///     The InitializeAccount instruction requires no signers and MUST be included within the same Transaction as the system
        ///     program's <see cref="SystemProgram.CreateAccount(string, string, long, long, string)"/> or <see
        ///     cref="SystemProgram.CreateAccount(byte[], byte[], long, long, byte[])"/> instruction that creates the account being
        ...
        public static TransactionInstruction InitializeAccount(string account, string mint, string owner)
        {
            return InitializeAccount(
                Encoders.Base58.DecodeData(account),
                Encoders.Base58.DecodeData(mint),
                Encoders.Base58.DecodeData(owner));
        }
        ...
        public static TransactionInstruction InitializeAccount(byte[] account, byte[] mint, byte[] owner)
        {
            var keys = new List<AccountMeta>
            {
                ...
                new AccountMeta(Encoders.Base58.DecodeData(SysvarRentPublicKey), false, false)
            };
            ...
            string freezeAuthority = "")
        {
            return InitializeMint(
                Encoders.Base58.DecodeData(mint),
                decimals,
                Encoders.Base58.DecodeData(mintAuthority),
                !string.IsNullOrWhiteSpace(freezeAuthority) ? Encoders.Base58.DecodeData(mintAuthority) : null);
        }
