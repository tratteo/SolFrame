...
namespace Solnet.Rpc.Models
{
    ...
    public class Message
    {
        ...
        {
            // Read message header
            var numRequiredSignatures = data[MessageHeader.Layout.RequiredSignaturesOffset];
            var numReadOnlySignedAccounts = data[MessageHeader.Layout.ReadOnlySignedAccountsOffset];
            var numReadOnlyUnsignedAccounts = data[MessageHeader.Layout.ReadOnlyUnsignedAccountsOffset];

            // Read account keys
            (var accountAddressLength, var accountAddressLengthEncodedLength) =
            ...
            for (var i = 0; i < accountAddressLength; i++)
            {
                var keyBytes = data.Slice(
            ...
            (var instructionsLength, var instructionsLengthEncodedLength) =
            ...
            var instructionsOffset =
                MessageHeader.Layout.HeaderLength + accountAddressLengthEncodedLength +
                (accountAddressLength * PublicKey.PublicKeyLength) + PublicKey.PublicKeyLength +
                instructionsLengthEncodedLength;
            var instructionsData = data[instructionsOffset..];

            // Read the instructions in the message
            for (var i = 0; i < instructionsLength; i++)
            {
                (var compiledInstruction, var instructionLength) =
        ...
        {
            var accountAddressesLength = ShortVectorEncoding.EncodeLength(AccountKeys.Count);
            var instructionsLength = ShortVectorEncoding.EncodeLength(Instructions.Count);
            var accountKeysBufferSize = AccountKeys.Count * 32;
            ...
            var messageBufferSize = MessageHeader.Layout.HeaderLength + PublicKey.PublicKeyLength +
            ...
            foreach (var compiledInstruction in Instructions)
            {
