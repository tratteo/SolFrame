    ...
    public class BIP
    {
        ...
        public string MnemonicToEntropy(string mnemonic, BIP39Wordlist wordlistType)
        {
            ...
            var bits = string.Join("", words.Select(word =>
            {
                ...
                return index == -1 ? throw new FormatException(InvalidMnemonic) : LPad(Convert.ToString(index, 2), "0", 11);
            }));
        ...
        private string Salt(string password) => "mnemonic" + (!string.IsNullOrEmpty(password) ? password : "");
        ...
        private string BytesToBinary(byte[] hash) => string.Join("", hash.Select(h => LPad(Convert.ToString(h, 2), "0", 8)));
        ...
        private string[] GetWordlist(BIP39Wordlist wordlist)
        {
            ...
            var stringToReturn = PlayerPrefs.GetString(wordListFile);
